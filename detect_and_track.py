...
# Add a global dictionary to store identities based on bounding box coordinates
global_identities = {}
start_time = time.time()
person_moved = 0

def is_close(box_key, threshold=100):
    # traverse on items in global_identities keys, which are also box coordinates
    # if center point of box_key is close to center point of traversed item, return True and coordinates of traversed item as tuple
    # if not, return False and box_key again 
    if len(global_identities) == 0:
        return False, box_key, True
    for existing_box_coord, _ in global_identities.items():
        # Check if the center points are very close
        existing_center = np.array([(existing_box_coord[0] + existing_box_coord[2]) / 2,
                                    (existing_box_coord[1] + existing_box_coord[3]) / 2])
        current_center = np.array([(box_key[0] + box_key[2]) / 2, (box_key[1] + box_key[3]) / 2])
...
def draw_boxes(img, bbox, identities=None, categories=None, names=None, save_with_object_id=False, path=None,offset=(0, 0)):
    ...
    for i, box in enumerate(bbox):
        ...
        if cat == 0:    
            box_key = (x1, y1, x2, y2)
            box_exist, existing_box_coord, first_person = is_close(box_key)
            if box_exist == False and first_person == True:
                obj_id = len(global_identities) + 1
                global_identities[box_key] = obj_id
            elif box_exist == True:
                if person_moved == 1:
                    person_moved = 0
                obj_id = global_identities[existing_box_coord]
                label = names[cat] +" id:"+ str(obj_id)
                (w, h), _ = cv2.getTextSize(label, cv2.FONT_HERSHEY_SIMPLEX, 0.6, 1)
                cv2.rectangle(img, (existing_box_coord[0], existing_box_coord[1]), (existing_box_coord[2], existing_box_coord[3]), (255,0,20), 2)
                cv2.rectangle(img, (existing_box_coord[0], existing_box_coord[1] - 20), (existing_box_coord[0] + w, existing_box_coord[1]), (255,144,30), -1)
                cv2.putText(img, label, (existing_box_coord[0], existing_box_coord[1] - 5),cv2.FONT_HERSHEY_SIMPLEX, 
                            0.6, [255, 255, 255], 1)
            elif box_exist == False and first_person == False:
                print("person moving")
                person_moved = 1
                start_time = time.time()
